...
export class MainPageComponent implements OnInit {
  ...
  getTopanime(pageno: any) {
    this.api.topAnime(pageno).subscribe(
      (res: any) => {
        (this.Topanime = res?.data), console.log(this.Topanime);
      },
      (err) => {
        console.log(err);
      }
  ...
  searchanime(name: any, page: any) {
    setTimeout(() => {
      this.api.searchable(name, page).subscribe(
        (res: any) => {
          (this.searchanimeresult = res.data), console.log(res);
        },
        (err) => {
          console.log(err);
        }
  ...
  addwatchlish(item: any, id: any, i: any) {
    ...
    this.api.listanime().subscribe((res: any) => {
      console.log(res), (this.wishlist = res);
    });
    console.log(id);
  ...
  onScroll() {
    // this.api.topAnime(++this.pageadd).subscribe((data: data[]) => {
